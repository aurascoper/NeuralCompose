...
- **Supporting hypotheses:** `Evaluation/corpora/dream_mode_hypothesis_registry.json` (4 pre-registered hypotheses: S-1 routing/cascades/policies schema, S-2 Python offline extraction + human-rated drift baseline, S-3 Random Forest sleep-stage classifier on synthetic data, S-4 Stage 4 Swift actor FSM deferred). S-3 has both milestones landed (synthetic generator + RF trainer, per-seed mean macro F1 0.9394 +- 0.0127 on synthetic, honest framing on in-distribution testing). S-2 has milestones 1, 2, and 3 landed (three-pass pipeline passed on synthetic rho 0.9856; FAILED rho >= 0.6 on human-rated fixture with the proxy: 0.1352; LLMDriftScorer backend wired to a real Ollama server with a DriftScoring protocol that preserves the importable API; urllib.request-based zero-deps client; JSON-mode response_format; multi-seed evaluation harness landed 2026-07-19). The full multi-seed bake-off across the qwen2.5 ladder and the r1 reasoning-distill (on n=6 human-rated) found: qwen2.5:0.5b mean -0.08 std 0.38 95% CI [-0.41, +0.25] (in the noise); qwen2.5:1.5b mean -0.09 std 0.24 [-0.30, +0.12] (in the noise); qwen2.5:3b mean +0.00 std 0.00 (deadlocked); deepseek-r1:1.5b + r1 prompt mean +0.27 std 0.56 [-0.36, +0.91] (R1 reasoning hypothesis falsified at 1.5B distill size, CI crosses zero). The only configuration that robustly clears rho >= 0.6 is the cloud `deepseek-v4-flash:cloud` (v1 prompt, 3-run mean rho +0.84 std 0.05 95% CI [+0.75, +0.93], mean|err| 0.12) - a network cloud model, NOT the spec candidate. **S-2 candidate decision — RESOLVED 2026-07-19 (user, this session; UPDATED same day to Sonnet 5):** adopt **Sonnet 5 via the `claude-cli` backend** — headless `claude -p` on the user's Max-plan subscription, **no API key and no per-call API billing** — as the S-2 drift scorer for the **OFFLINE research/eval tool ONLY**. Rationale: Sonnet 5 is statistically **tied** with `deepseek-v4-flash:cloud` on accuracy (Sonnet 5 3-run rho 0.83 ± 0.06 vs deepseek 0.91 ± 0.02, CIs overlap at n=6; both robustly clear 0.6), so the deciding factor is auth/ops — Sonnet 5 needs no API key and no Ollama cloud gateway, and is a first-class model the user already controls. `deepseek-v4-flash:cloud` (nominally higher rho, via the existing Ollama signin) remains the recorded tied alternative. Both are **network models**, explicitly barred from the on-device Swift runtime (`Sources/BCILLM/`); the scorers log an OFFLINE-EVAL-ONLY warning (the guard fires for any `:cloud` model, remote base_url, api_key, or the `claude` CLI backend). The originally-intended on-device replacement `deepseek-r1:1.5b` (DeepSeek-R1-Distill-Qwen-1.5B) was pulled and tested under the r1 prompt topology (think tags + tail JSON) and is empirically undersized (mean rho +0.27 std 0.56, R1 reasoning falsified at 1.5B distill size). Consequently **on-device runtime drift scoring remains UNSOLVED** — the deterministic proxy (rho 0.135, zero variance, fully offline) is the only on-device baseline. Provenance note: commit 1a265cf's '+0.84 deepseek' line is this **cloud** model, easily mistaken for the local deepseek-r1:1.5b. If an on-device path is required later, the queued (un-adopted) candidates are larger R1 distills (deepseek-r1:7b/8b/14b/32b, ~4-20 GB q4_K_M disk) or non-deepseek reasoning families (qwen3, etc.) — but the **binding constraint is the n=6 fixture size, not model choice**, so growing the human-rated fixture (with multi-rater consensus) precedes any further model bake-off. The R1 prompt + regex tail-extractor is preserved on disk for the falsification record. **[hybrid bake-off, 2026-07-19]** The user's "joint-embedding + LLM cross-examiner" design (a bge-small distance injected into the cloud LLM's prompt as a prior) was built (`--backend hybrid`/`bge`; `HybridDriftScorer`/`BgeCosineScorer`; auth-capable OpenAI-compat client so Ollama-cloud OR Claude can fill the slot) and benchmarked — result: **FALSIFIED**. Text-alone deepseek-v4-flash:cloud scores 3-run mean rho **0.91**; the hybrid (same LLM + bge prior) drops to **0.43–0.54** across v1/v2 variants. The embedding prior is much weaker than the LLM's own text+anchor reasoning and drags the strong reasoner toward the weaker ranking; a *better* prior (contrastive v2 > raw v1 as a standalone signal) did not rescue it — any embedding prior distracts. **Winner: text-alone deepseek-v4-flash:cloud (offline-eval-only).** Best pure-embedding, on-device-capable signal found: **contrastive bge-cosine** (report vs its own vs the competing hypothesis's anchor text) at rho **0.37** — a real gain over the symbolic proxy (0.135) but still below the 0.6 bar, so **on-device drift scoring remains UNSOLVED**. Bug fixed en route: the example hypotheses carry an empty `target_concept`, so every pre-hybrid bge cosine had been embedding an empty string (the earlier bge-alone rho ≈ 0 was that artifact, not a verdict); the signal now falls back to anchor text. The Claude arm was then run **text-alone via the `claude` CLI on the user's Max-plan subscription** (new `--backend claude-cli`, headless `claude -p` with `--system-prompt` replacing the agent prompt — no API key, no per-call API billing): **Sonnet 5 scores 3-run mean rho 0.83 ± 0.06 (CI [0.77, 0.90]), mean|err| 0.17** — robustly clears 0.6 and is **statistically tied** with deepseek-v4-flash:cloud (0.91 ± 0.02; CIs overlap at n=6), so the two cloud text-alone scorers are effectively equivalent on accuracy and the choice is auth/ops (Sonnet 5 needs no key/gateway). The hybrid stays moot for Claude too — the bge prior degrades any strong reasoner. S-4 still pre-registered.
...
<!-- Last updated: 2026-07-19T09:52:18Z (S-2 scorer UPDATED to Sonnet 5 via claude-cli/Max-plan no-key; deepseek-v4-flash:cloud tied alternative; hybrid falsified; on-device unsolved) -->
