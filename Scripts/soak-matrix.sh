...
for path in sorted(glob.glob(os.path.join(base_dir, "baselines", "*.json"))):
    ...
    matrix["cells"].append({
        ...
        "metrics": {
            ...
            "synthesis_rate": (data.get("outcome_counts", {}) or {}).get("synthesis", 0) / max(1, data.get("turn_count", 0)),
            "coherence_rate": (data.get("outcome_counts", {}) or {}).get("coherence-seeking", 0) / max(1, data.get("turn_count", 0)),
            "displacement_rate": (data.get("outcome_counts", {}) or {}).get("displacement-seeking", 0) / max(1, data.get("turn_count", 0)),
