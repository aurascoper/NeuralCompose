# EEG Integration Design Notes

**Status: Phase 1 data collection landed 2026-07-18; everything beyond it remains unscheduled.** The app can now collect explicit, local, paired EEG-feature/action transitions through the independently opt-in `TransitionCaptureManager` described in `ADR-006-jepa-transition-capture.md`. This does not add a runtime inference process or commit the app to a JEPA/MPC product path.
...
**The former gap was narrower than "build a windower":** nothing persisted `(window, action, next_window)` tuples to disk in a form an offline PyTorch trainer could load. That gap is now filled by `JEPASpectralStateRingBuffer` and `TransitionCaptureManager`: they derive a compact feature state from the existing live window, hold a fixed chronological pre/post buffer, and append JSONL only after the dedicated capture toggle is enabled.

**The gap was also a documented, deliberate decision.** `TelemetryEvent` remains the narrow, opt-in interaction log specified by `ADR-005`; it was not widened. `ADR-006` records the separate consent, schema, and visible-recording boundary for the new feature-state data set, which is why this capture path is not a hidden extension of ordinary interaction logging.
...
**It still has two real prerequisites:** Phase 1 now provides a persisted, offline-loadable dataset path, but it does not yet provide collection volume; Phase 2 still needs a trained and independently validated continuous encoder. Calibration cannot happen before both a sufficient corpus and that validation exist.
...
|---|---|
| Sliding-window `(Channels, Time)` ingestion | **Implemented for collection.** `EEGWindowing`/`EEGWindow` provide the live 4ch/2s/256Hz input; the opt-in `TransitionCaptureManager` now persists paired feature-window/action/feature-window JSONL for offline training. |
